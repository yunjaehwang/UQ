...
% num_iter    = 1;       % # of total number of iteration
% num_sample  = 10;      % # of samples in each iteration
...
data.q_wall_in  = zeros(num_sample*num_iter, len_time);
data.q_wall_out = zeros(num_sample*num_iter, len_time);
data.q_roof_in  = zeros(num_sample*num_iter, len_time);
data.q_roof_out = zeros(num_sample*num_iter, len_time);

data.convection_wall_in  = zeros(num_sample*num_iter, len_time); 
data.convection_wall_out = zeros(num_sample*num_iter, len_time);
data.convection_roof_in  = zeros(num_sample*num_iter, len_time);
data.convection_roof_out = zeros(num_sample*num_iter, len_time);
data.convection_floor    = zeros(num_sample*num_iter, len_time);

data.radiation_wall_in  = zeros(num_sample*num_iter, len_time);
data.radiation_wall_out = zeros(num_sample*num_iter, len_time);
data.radiation_roof_in  = zeros(num_sample*num_iter, len_time);
data.radiation_roof_out = zeros(num_sample*num_iter, len_time);
data.radiation_rtof = zeros(num_sample*num_iter, len_time);
data.radiation_solar = zeros(num_sample*num_iter, len_time);
data.radiation_floor = zeros(num_sample*num_iter, len_time);

data.q_in = zeros(num_sample*num_iter, len_time);
data.q_nv = zeros(num_sample*num_iter, len_time);
...
for iter = 1:num_iter
    ...
    for i =1:num_sample
        ...
        try
            data_temp = dlmread(f_out_raw,',',1,0);
            data.temp_in(idx,:) = data_temp(:,226)';
            data.wall(idx,:) = data_temp(:,137)';
            data.wall_in(idx,:) = data_temp(:,30)';
            data.wall_out(idx,:) = data_temp(:,33)';
            data.roof(idx,:) = data_temp(:,131)';
            data.roof_in(idx,:) = data_temp(:,24)';
            data.roof_out(idx,:) = data_temp(:,27)';
            data.vent_rate(idx,:) = data_temp(:,288)';
            
            data.q_wall_in(idx,:)  = data_temp(:,28)';
            data.q_wall_out(idx,:) = data_temp(:,31)';
            data.q_roof_in(idx,:)  = data_temp(:,22)';
            data.q_roof_out(idx,:) = data_temp(:,25)';
            
            data.convection_wall_in(idx,: ) = data_temp(:,53)';
            data.convection_wall_out(idx,: ) = data_temp(:,56)';
            data.convection_roof_in(idx,: ) = data_temp(:,48)';
            data.convection_roof_out(idx,: ) = data_temp(:,51)';
            data.convection_floor(idx,: ) = data_temp(:,46)';
            
            data.radiation_wall_in(idx,: ) = data_temp(:,82)';
            data.radiation_wall_out(idx,: ) = data_temp(:,84)';
            data.radiation_roof_in(idx,: ) = data_temp(:,76)';
            data.radiation_roof_out(idx,: ) = data_temp(:,78)';
            data.radiation_rtof(idx,: ) = data_temp(:,80)';
            data.radiation_solar(idx,: ) = data_temp(:,72)';
            data.radiation_floor(idx,: ) = data_temp(:,74)';
            
            data.q_in(idx,:) = data_temp(:,71)';
            data.q_nv(idx,:) = data_temp(:,106)';
        catch
            warning(['Error reading in',num2str(idx),'File']);
...
data2=data;
idx = (data.vent_rate(:,1) ~= 0);
%%
load('/home/yunjaeh/OpenFOAM/yunjaeh-4.0/run/UQ/uq_2019/integral_time_stamp.mat');     % load time stamp


data.temp_in = data2.temp_in(idx,:);
data.wall = data2.wall(idx,:);
data.wall_in = data2.wall_in(idx,:);
data.wall_out= data2.wall_out(idx,:);
data.roof = data2.roof(idx,:);
data.roof_in = data2.roof_in(idx,:);
data.roof_out = data2.roof_out(idx,:);
data.vent_rate = data2.vent_rate(idx,:);

data.q_wall_in = data2.q_wall_in(idx,:);
data.q_wall_out = data2.q_wall_out(idx,:);
data.q_roof_in = data2.q_roof_in(idx,:);
data.q_roof_out = data2.q_roof_out(idx,:);

data.convection_wall_in = data2.convection_wall_in(idx,:);
data.convection_wall_out = data2.convection_wall_out(idx,:);
data.convection_roof_in= data2.convection_roof_in(idx,:); 
data.convection_roof_out= data2.convection_roof_out(idx,:);
data.convection_floor= data2.convection_floor(idx,:);

data.radiation_wall_in = data2.radiation_wall_in(idx,:);
data.radiation_wall_out = data2.radiation_wall_out(idx,:);
data.radiation_roof_in = data2.radiation_roof_in(idx,:);
data.radiation_roof_out = data2.radiation_roof_out(idx,:);
data.radiation_rtof = data2.radiation_rtof(idx,:);
data.radiation_solar = data2.radiation_solar(idx,:);
data.radiation_floor = data2.radiation_floor(idx,:);

data.q_in = data2.q_in(idx,:);
data.q_nv = data2.q_nv(idx,:);

%
figure();
plot(t_stamp, mean(data.temp_in)-273.15);
xticks(0:12:24);
...
plot(t_stamp, mean(data.convection_roof_in));
plot(t_stamp, mean(data.convection_wall_in));
plot(t_stamp, mean(data.convection_floor));
title('Convection');
xticks(0:12:24);
xticklabels({'Noon','Midnight','Noon'});
ylabel('q [W]');
legend('Roof','Wall','Floor');


subplot(1,2,2); hold on
plot(t_stamp, mean(data.radiation_roof_in));
plot(t_stamp, mean(data.radiation_rtof));
plot(t_stamp, mean(data.radiation_wall_in));
plot(t_stamp, mean(data.radiation_floor));
title('Radiation');
xticks(0:12:24);
xticklabels({'Noon','Midnight','Noon'});
ylabel('q [W]');
legend('Roof','Roof to floor','Wall','Floor');


figure(); hold on
% plot(t_stamp, mean(data.q_wall_in));
plot(t_stamp, mean(data.q_roof_in));
plot(t_stamp, mean(data.q_wall_in));
plot(t_stamp, mean(data.convection_floor + data.radiation_floor));
plot(t_stamp, mean(data.q_in));
plot(t_stamp, mean(data.q_nv));
xticks(0:12:24);
xticklabels({'Noon','Midnight','Noon'});
ylabel('q [W]');
legend('q_{roof}','q_{wall}','q_{floor}','q_{infiltration}','q_{natural ventilation}');

%% 0701
% plot mean temperature
figure(); 
subplot(1,2,1); hold on
plot(t_stamp, std(data.convection_roof_in));
plot(t_stamp, std(data.convection_wall_in));
plot(t_stamp, std(data.convection_floor));
title('Convection');
xticks(0:12:24);
xticklabels({'Noon','Midnight','Noon'});
ylabel('q [W]');
...
subplot(1,2,2); hold on
plot(t_stamp, std(data.radiation_roof_in));
plot(t_stamp, std(data.radiation_rtof));
plot(t_stamp, std(data.radiation_wall_in));
plot(t_stamp, std(data.radiation_floor));
title('Radiation');
xticks(0:12:24);
xticklabels({'Noon','Midnight','Noon'});
ylabel('q [W]');
...
figure(); hold on
% plot(t_stamp, mean(data.q_wall_in));
plot(t_stamp, std(data.q_roof_in));
plot(t_stamp, std(data.q_wall_in));
plot(t_stamp, std(data.convection_floor + data.radiation_floor));
plot(t_stamp, std(data.q_in));
plot(t_stamp, std(data.q_nv));
xticks(0:12:24);
xticklabels({'Noon','Midnight','Noon'});
ylabel('q [W]');
legend('q_{roof}','q_{wall}','q_{floor}','q_{infiltration}','q_{natural ventilation}');
...
%%
% plot(t_stamp, mean(data.q_wall_out)-273.15);
figure(); hold on
plot(t_stamp, mean(data.temp_in)+std(data.temp_in)-273.15);
plot(t_stamp, mean(data.temp_in)-273.15);
plot(t_stamp, mean(data.temp_in)-std(data.temp_in)-273.15);
